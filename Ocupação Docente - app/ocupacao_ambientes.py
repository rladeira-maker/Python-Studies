...
def findString(start,sheet, string):
    ...
    for coluna in range(start[0],30): #colunas com nomes 1 e 15
        for linha in range(start[1],1000): #linhas com nomes começa com 1 e a cada 20
            #print(coluna,linha)
...
def preenche_ambientes(wb2,count,index,qtd_aulas,aulas):
    ...
    while count > 0:
        for x in range (10):
            docente1 = aulas[index[a]][diasDaSemana[x],'amb1']
            docente2 = aulas[index[a]][diasDaSemana[x],'amb2']
...
def main():
    ...
    try:
        ...
        wb3 = openpyxl.load_workbook('Ambientes_de_ensino.xlsx')
    except:
        print ('Erro. Os arquivos  Ambientes_de_ensino.xlsx e GERAL.xlsx estão na mesma pasta do programa?')

    horario = len(wb1.sheetnames)
    ...
    # - - - - -
        ...
        wb3 = preenche_ambientes(wb3,count,index,qtd_aulas,aulas)
    try:
        wb2.save('Ocupacao_docentes.xlsx')
        wb3.save('Ocupacao_ambientes.xlsx')
